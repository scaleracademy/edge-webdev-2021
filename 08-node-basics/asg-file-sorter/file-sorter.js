...
for (i in data1) sortedData.push(parseInt(data1[i]));
for (i in data2) sortedData.push(parseInt(data2[i]));
for (i in data3) sortedData.push(parseInt(data3[i]));
...
for (i = 0; i < sortedData.length; i++){
  newData += sortedData[i];
  if(i<sortedData.length-1){
    newData += "\n";
  }
}
fs.writeFileSync("./sorted.txt", newData, (err) => {
  if (err) throw err;
